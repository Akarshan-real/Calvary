...
export interface FuseButtonProps {
  ...
  settle?: FuseSettle;
  disabled?: boolean;
  beforeArm?: () => boolean | Promise<boolean>;
...
const FuseButton: React.FC<FuseButtonProps> = ({
  ...
  latest.current = { beforeArm, onCommit, onUndo, onFuseEnd, onPhaseChange, commitOn, settle };
  ...
  const arm = async () => {
    ...
    if (latest.current.beforeArm) {
      const canProceed = await latest.current.beforeArm();
  ...
  return (
    <span
      ref={rootRef}
      tabIndex={-1}
      className={`group relative inline-grid h-[var(--fb-h)] grid-cols-[minmax(0,1fr)] isolate touch-manipulation select-none overflow-hidden rounded-[var(--fb-radius)] font-medium leading-none tracking-[0.01em] outline-none [font-family:inherit] [font-size:var(--fb-fs)] [background:var(--fb-bg)] [color:var(--fb-ink)] border border-black/80 shadow-[0_4px_20px_rgba(0,0,0,0.5),0_0_12px_rgba(255,190,51,0.25)] ring-1 ring-black/60 [-webkit-touch-callout:none] [-webkit-tap-highlight-color:transparent] [transition:transform_160ms_var(--fb-ease-out),box-shadow_200ms_ease] hover:shadow-[0_6px_24px_rgba(0,0,0,0.6),0_0_18px_rgba(255,190,51,0.4)] data-[pressed]:[transform:scale(var(--fb-press))] motion-reduce:data-[pressed]:[transform:none] has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-solid has-[:focus-visible]:outline-offset-[3px] has-[:focus-visible]:[outline-color:color-mix(in_srgb,var(--fb-ink)_60%,transparent)] data-[phase=idle]:has-[.fb-idle:disabled]:opacity-[0.55]${className ? ` ${className}` : ''}`}
      data-phase={phase}
      data-fuse={fuse}
