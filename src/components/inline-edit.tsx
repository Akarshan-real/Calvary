...
import React, { useState, useId, type FC } from 'react';
...
export const EditableRow: FC<EditableRowProps> = ({
  ...
  return (
    <motion.div
      layout
      transition={spring}
      className={`relative flex w-full ${multiline ? 'flex-col items-start gap-3' : 'flex-col gap-2 sm:flex-row sm:items-center sm:gap-4'}`}
    >
      <div
        className={`flex shrink-0 items-center gap-3 ${multiline ? 'w-full' : 'w-full sm:w-[150px]'}`}
      >
        <HugeiconsIcon
          icon={icon}
          size={22}
        ...
        <label
          htmlFor={inputId}
          className="cursor-pointer text-[14px] font-semibold text-neutral-300 uppercase tracking-wider"
        >
          {label}
        </label>
      </div>
      ...
      <div
        className={`relative w-full bg-transparent transition-colors`}
      >
        <motion.div
          layout
